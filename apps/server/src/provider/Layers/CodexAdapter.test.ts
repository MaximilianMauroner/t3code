...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("returns fresh cached account rate limits before starting a no-session runtime", () => {
    const isolatedRuntimeFactory = makeRuntimeFactory({
    ...
    return Effect.gen(function* () {
      ...
      const runtimeCountBeforeNoSessionRead = isolatedRuntimeFactory.runtimes.length;

      const snapshot = yield* adapter.readCodexUsage!();

      assert.equal(isolatedRuntimeFactory.runtimes.length, runtimeCountBeforeNoSessionRead);
      assert.equal(snapshot?.source, "cache");
      assert.deepStrictEqual(snapshot?.windows[0], {
        kind: "five-hour",
        usedPercent: 25,
