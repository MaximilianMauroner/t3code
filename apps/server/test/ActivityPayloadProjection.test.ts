...
describe("projectActivityPayload", () => {
  ...
  it("preserves omitted-history semantics while keeping mobile details lazy", () => {
    const omitted = {
      ...fixtures[0]!,
      payloadOmitted: true,
    } satisfies OrchestrationThreadActivity;
    const projected = projectActivityPayload(omitted);
    const [group] = buildThreadFeed(makeThread([projected]));
    ...
    expect(group.activities[0]?.getCopyText()).toBe("Completed command_execution");
  });
